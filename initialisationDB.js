...
async function initDB() {
    ...
    try {
        ...
        await pool.query(`Drop table Metier, utilisateur, inventaire, Fabrication`);
        ...
        await pool.query(`CREATE TABLE IF NOT EXISTS Vente (id_vente SERIAL PRIMARY KEY, prix float, quantite int,fk_ressource int, fk_utilisateur int, foreign key(fk_utilisateur) references utilisateur(id_utilisateur), foreign key (fk_ressource) references Ressource(id_ressource));`);
        */
        ...
        await pool.query(`update inventaire set quantité = 2500 where fk_utilisateur = 1`);
...
async function testDb() {
    try {
        const resultat = await pool.query(`Select * from utilisateur`);
